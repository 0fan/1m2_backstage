...
(function(){
	var AccountMap = {
		...
		622245: "中国工商银行.贷记卡",
		622245: "中国工商银行.贷记卡",
		622250: "交通银行股份有限公司太平洋双币信用卡中心.太平洋人民币贷记卡",
		622250: "交行太平洋卡中心.太平洋人民币贷记卡",
		622251: "交通银行股份有限公司太平洋双币信用卡中心.太平洋人民币贷记卡",
		622251: "交行太平洋卡中心.太平洋人民币贷记卡",
		622252: "交通银行股份有限公司太平洋双币信用卡中心.太平洋人民币贷记卡",
		622252: "交行太平洋卡中心.太平洋人民币贷记卡",
		622253: "交通银行股份有限公司太平洋双币信用卡中心.太平洋人民币贷记卡",
		...
		622400: "营口市商业银行.辽河一卡通",
		622406: "香港上海汇丰银行有限公司.ATMCard",
		622406: "香港上海汇丰银行有限公司.ATMCard",
		622407: "香港上海汇丰银行有限公司.ATMCard",
		622407: "香港上海汇丰银行有限公司.ATMCard",
		622411: "威海市商业银行.通达卡",
		622411: "威海市商业银行.通达卡",
		622412: "湖北农信社.信通卡",
		...
		622506: "陕西省农村信用社联合社.陕西信合富泰卡",
		622509: "高要市农村信用合作社联合社.信通白金卡",
		622509: "高要市农村信用合作社联社.信通白金卡",
		622510: "高要市农村信用合作社联合社.信通金卡",
		...
		622836: "农业银行.金穗贷记卡",
		622837: "农业银行.金穗贷记卡",
		622840: "农业银行.金穗通宝卡（暂未使用）",
		622841: "农业银行.金穗惠农卡",
		622842: "农业银行.金穗通宝卡（暂未使用）",
		622843: "农业银行.金穗通宝贵宾卡(银)",
		622844: "农业银行.金穗通宝卡（暂未使用）",
		622845: "农业银行.金穗通宝贵宾卡（金）",
		622846: "农业银行.金穗通宝贵宾卡（白金）",
		622847: "农业银行.金穗通宝卡（单位卡）",
		622848: "农业银行.金穗通宝卡（个人普卡）",
		622849: "农业银行.金穗通宝贵宾卡（钻石）",
		622855: "江苏东吴农村商业银行.新苏卡",
		622856: "桂林市商业银行.漓江卡",
		622857: "日照市商业银行.黄海卡",
		622858: "浙江省农村信用社联合社.丰收卡",
		622859: "珠海农村信用合作社联社.信通卡",
		622860: "大庆市商业银行.玉兔卡",
		622864: "莱芜市商业银行.金凤卡",
	...
	Card.prototype = {
		render:function(cfg){
			...
			CFG.obj = $(CFG.obj);
			if(!(CFG.obj).length){ /*如果传值无效则不做操作*/
				return false;
			}

			var card_tip = $("<div class='mod-cardTip'></div>");
			CFG.obj.on("focus",function(){
				card_tip.appendTo("body");
				card_tip.show();
				axis(CFG.obj,card_tip);
			});
			CFG.obj.on("blur",function(){
				card_tip.hide();
			});
			CFG.obj.on("keyup",function(){
				var txt = $(this).val();
				!txt ? card_tip.hide() : card_tip.show();
				var card_tip_items = "<ul class='mod-cardTip-wrap'>";
				for(var i=0,len=txt.length;i<len;i++){
					card_tip_items += "<li class='mod-cardTip-item'>" + txt[i] + "</li>";
				}
				card_tip_items += "</ul>";
				card_tip.html(card_tip_items);
				axis(CFG.obj,card_tip);

				if(valid(txt)){
					if(CFG.bankPlace && (typeof AccountMap[txt.substr(0,6)]) != "undefined"){
						if($(CFG.bankPlace).is("input")){
							$(CFG.bankPlace).val(AccountMap[txt.substr(0,6)]);
						}else{
							$(CFG.bankPlace).text(AccountMap[txt.substr(0,6)]);
						}
					}
				}else{
					if($(CFG.bankPlace).is("input")){
			...
			/*计算位置*/
			function axis(obj,self){
				var w  = obj.outerWidth();
				var h  = obj.outerHeight();
				var x  = obj.offset().left;
				var y  = obj.offset().top;
				var _w = self.outerWidth();
				var _h = self.outerHeight();
				self.css({
					left : x + (w - _w) /2 + "px",
					top : y - _h - 30 + "px"
				});
			...
			*/
			function valid(num){ 
				num = String(num);
				if( !(/^[1-9]+[0-9]*]*$/.test(num)) || !(num.length > 15 && num.length < 20)){ 
					return false;
				}else{
					return true;
				}
